...
export const AutoGenerateModal = ({
  ...
  const cuisines = ["Italian", "Mexican", "Asian", "Indian", "American", "Mediterranean", "French"];
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-zinc-900/60 backdrop-blur-sm animate-in fade-in">
      <div className="bg-white dark:bg-zinc-900 rounded-[2.5rem] p-8 max-w-lg w-full shadow-2xl border border-zinc-200 dark:border-zinc-800 flex flex-col gap-6">
        ...
        <div className="space-y-6 max-h-[60vh] overflow-y-auto pr-2 custom-scrollbar">
          ...
          <div className="space-y-3">
            ...
            <div className="flex flex-wrap gap-2">
              {cuisines.map(c => (
                <button
                  key={c}
                  onClick={() => setPreferences(p => ({ ...p, cuisinePreferences: toggleSelection(p.cuisinePreferences, c) }))}
          ...
          <div className="space-y-3">
            ...
            <div className="flex flex-wrap gap-2">
              {diets.map(d => (
                <button
                  key={d}
                  onClick={() => setPreferences(p => ({ ...p, dietaryRestrictions: toggleSelection(p.dietaryRestrictions, d) }))}
                  className={`px-4 py-2 rounded-xl text-sm font-bold transition-all border ${
           ...
           <div className="space-y-3">
            ...
            <div className="flex bg-zinc-100 dark:bg-zinc-800/50 p-1 rounded-2xl">
              {[ { l: 'Any', v: undefined }, { l: 'Easy', v: 0 }, { l: 'Medium', v: 1 }, { l: 'Hard', v: 2 } ].map(lvl => (
                <button
                  key={lvl.l}
                  onClick={() => setPreferences(p => ({ ...p, maxDifficulty: lvl.v }))}
          ...
          <div className="space-y-3">
            <label className="text-sm font-bold text-zinc-500 dark:text-zinc-400 uppercase tracking-wider flex justify-between">
              ...
              <span className="text-indigo-500">{preferences.maxTotalTimeMinutes || 'Any'}</span>
            </label>
            <input 
              type="range" 
              min="10" 
              ...
              onChange={(e) => setPreferences(p => ({ ...p, maxTotalTimeMinutes: parseInt(e.target.value) === 180 ? undefined : parseInt(e.target.value) }))}
        ...
        >
          ...
          <span>{isGenerating ? 'Generating Magic...' : 'Generate Meal Plan'}</span>
        </button>
      </div>
    </div>,
  document.body
